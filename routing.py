...
class routing:
	def __init__(self, member, nodeid):
	...
	def propagateUpdate(self, newroute):
		for node in neighbors:
			sendData(pickle(self.nodeid,newroute))

	def checkRoute(self, route):#assume route updates are like (senderID, [(revnoceid, prevhopdistance),(,)])
		nodeid=route[0]
		nodevec=route[1]
		#update routestore
	...
	def computeGraph(self, bestroutes):
		G = nx.Graph()
	...
	def computeSpanningTree(self, graph):
		T = nx.minimum_spanning_tree(G)
		return T

	def updateFwdTable(self. spantree, fwdtable):
		pass
