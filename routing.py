...
def handle_route_vector(incoming_node, route_vector):
	...
	for (node, ip_address, delay) in route_vector:
		if node == this_node:	# handles the loop case
			break
...
def get_next_node_to(dest):
    if this_node in fwd_table and dest in fwd_table[this_node]:
        return fwd_table[this_node][dest][1]  #fwd table of the format [source, A, B, C, dest]
    return None

def get_fwd_table(node):
	return nx.shortest_path(g, source=node)
